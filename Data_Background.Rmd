...
lm_hospitals1 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_5mi + typical_patient_complexity + `Hospital overall rating` + 
...
lm_hospitals2 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_10mi + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
lm_hospitals3 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_25mi + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
lm_hospitals4 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_hsa_total + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
lm_hospitals5 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_hrr_total + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
lm_hospitals6 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_hsa_procedure + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
lm_hospitals7 <- lm(data = primary_regression_data,
                   formula = percent_medicare ~ n_hosp_hrr_procedure + typical_patient_complexity + `Hospital overall rating` + 
                     median_household_income + percent_pop_over65 + pop_density +
                     Anthem_NH + Harvard_Pilgrim_HC + CIGNA + factor(procedure))
...
where $\zeta_i$ replaces the summation in Equation (2) and denotes insurer fixed effects; $\delta_j$ denotes procedure fixed effects as in Equation (2); $m_h$ represents the number of hospitals in hospital $h$'s market and $x_h$ is a vector of hospital characteristics such as hospital quality (from CMS Hospital Compare data), median household income in the hospital's county, percentage of person's over 65 years of age in the hospital's county, and the population density of the hospital's county ( [2017 US Census Data](https://www.census.gov/quickfacts/fact/map/US/INC110217) ).

A hospital's market is defined differently across several specifications. Two common definitions of a hospital's market in the literature are it's Hospital Service Area (HSA) and it's Hospital Referral Region (HRR), both defined and made publicly available by the [Dartmouth Atlas of Health Care](http://archive.dartmouthatlas.org/tools/downloads.aspx?tab=39). An HSA dilineates a local market for hospital care, composed of a collection of ZIP codes whose residents receive most of their hospitalizations from the hospitals in that area. Most HSAs contain only one hospital. An HRR dilineates regional health care markets for tertiary medical care and each contains at least one hospital that performs major cardiovascular procedures and neurosurgery. HRRs were largely defined by assigning HSAs to the region where the greatest proportion of major cardiovascular procedures were performed. My dataset contains 29 unique HSAs and 4 unique HRRs.

For both the HSA and HRR market definitions, I calculate the total number of hospitals within each hospital's market as well as the number of hospitals that also provide the same service within the market. 
...
stargazer(lm_insurers, type = "html", omit = c(4:172), 
...
stargazer(lm_hospitals1, lm_hospitals2, lm_hospitals3, lm_hospitals4, lm_hospitals5, lm_hospitals6, lm_hospitals7, type = "html", 
          ...
          covariate.labels = c("Number of Hospitals in 5 mile radius",
                               ...
                               "Total Number of Hospitals in HSA",
                               "Total Number of Hospitals in HRR",
                               "Number of Hospitals in HSA offering procedure",
                               "Number of Hospitals in HRR offering procedure",
                               "Typical Patient Complexity - Low",
                               "Typical Patient Complexity - Medium", 
                               "Overall Hospital Rating - 3",
                               "Overall Hospital Rating - 4",
                               "Overall Hospital Rating - 5",
                               "Overall Hospital Rating - Not Available",
                               "Median Household Income (county)",
                               "Percentage of the Population 65 and older (county)",
