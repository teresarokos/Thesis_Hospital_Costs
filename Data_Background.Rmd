...
primary_data$insurer <- str_replace(primary_data$insurer, "None", "Uninsured")
...
RVU18_info <- read_xlsx("RVU18A_files/PPRRVU18_JAN_mod.xlsx", skip = 9, col_names = TRUE) %>% 
  ...
  semi_join(primary_data, by = "cpt_code") %>% 
  filter(is.na(MOD)) %>% 
...
primary_data %>% 
  bind_rows(medicare_prices) %>% 
  filter(insurer == "Medicare") %>% 
  count(cpt_code, procedure, estimate_of_total_cost)
