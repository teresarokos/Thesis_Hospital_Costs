...
primary_data <- primary_data %>% 
  ...
  mutate(`Hospital Type` = case_when(is.na(`Hospital Type`) ~ "Non-Hospital Provider",
                                     TRUE ~ `Hospital Type`))
...
plotly_nh_hospitals <- primary_data %>%
  ...
  arrange(`Hospital Type`)
  plot_mapbox(lat = ~lat, lon = ~long, text = ~provider_name, color = ~`Hospital overall rating`,
