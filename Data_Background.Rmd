...
ggplot(data = health_expenditures) +
  geom_line(mapping = aes(x = year, y = percent_nhe)) +
  geom_line(mapping = aes(x = year, y = percent_hce, color = "red")) +
  geom_line(mapping = aes(x = year, y = percent_pcse, color = "blue")) +
  geom_line(mapping = aes(x = year, y = percent_pde, color = "green")) +
...
plot_geo(data = hospital_info, locationmode = 'USA-states', sizes = c(50, 300),
           mode = "markers") %>% 
    add_trace(
      x = ~longitude, y = ~latitude,
  hoverinfo = "text",
      text = ~paste(hospital_info$provider_name)
    ) %>%
    layout(geo = g) %>%
