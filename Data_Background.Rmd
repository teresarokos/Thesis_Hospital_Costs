...
primary_data$insurer <- str_replace_all(primary_data$insurer, "None", "Uninsured")
...
primary_data$provider_name <- str_replace_all(primary_data$provider_name, "Lakes Region General Healthcare", "Lakes Region General Hospital")
...
hospital_compare_data <- read_csv("Hospital_General_Information_copy.csv") %>% 
...
In standard micro-economic theory, one learns that prices reflect supply and demand, where the supply curve is largely determined by each firm's costs of production and market power and the demand curve determined by consumer preferences and budget constraints. Market power is largely a function of the number of other firms in the market, but a firm may also generate market power for itself through distiguishing it's product from others, most commonly in the form of quality. When it's product differs from others in the market, a firm can extract a certain mark-up from consumers who have a preference for that difference, and consumers often have a preference for higher quality. Thus, in a standard competitive market, one would expect prices to reflect production costs and product quality.

The market for health services (often provided through hospitals) is unique in that there are two components of demand: individuals and payers. Even this is somewhat of an oversimplification in the US market because many individuals are privately insured through their employer which contracts with a private insurer/payer. However, for the most part, insurers cover much of the costs of care ***citation*** and negotiate contracts with providers if they are private (Medicare and Medicaid set their prices at a regional level). This gives private payers some form of monopsony power as there are often only a few of them present in each market, and they can choose whether or not to include each provider in their network which affects the prices individuals face for care. 

Thus, payers and providers engage in bilateral negotiations where both are price setters rather than price takers. I assume the following: both the provider and the payer want to maximize their profit margins per patient, even if the provider is a non-profit (Horwitz 2005), and the number of patients they serve. Providers want to receive higher prices for their services but also to be included in payers' networks, as patients face lower out-of-pocket costs when going to providers in network. Payers want to pay lower prices for providers’ services to minimize their costs but also want to attract enrollees by including convenient providers in their network. Thus, this is the bargaining environment in which bilateral hospital-insurer contracts are negotiated and the work below examines the impact on prices.


### Background on the New Hampshire Market
#### Health Insurance Coverage
New Hampshire is a small state (8,953 sqaure miles) with a population of 1.343 million (90.5% white) and median household income of $68,485 (Census Bureau 2017). The following pie charts depict the insurance status of New Hampshire residents in 2016 as well as the market share of major private insurers. According to the New Hampshire Insurance Department, a total of 543,900 members were enrolled in commercial market plans in 2017 in the state, and 81% received coverage through employer-sponsored insurance plans (11.0% Individual Market, 7.7% NH PAP, 12.6% Small Group, 19.8% Large Group Fully Insured, and 48.9% Large Group Self Insured). 
```{r, echo=FALSE}
...
insurance_status_nh %>% 
  ...
  geom_text(aes(label = paste0(percent_in, "%")), position = position_stack(vjust = 0.5), color = "white") +
...
private_insurance %>% 
  ...
  geom_text(aes(label = paste0(percent_enrolled, "%")), position = position_stack(vjust = 0.5), color = "white") +
  labs(title = "Distribution by Insurer for NH Commercial Market in 2017")
```


New Hampshire has a total of 31 hospitals and 3,503 beds (New Hampshire Hospital Association 2018). According to the New Hampshire Hospital Association, 13 of these hospitals with 2,704 beds total are Prospective Payment Systems (PPS) Hospitals, which take prospective payments from Medicare; 13 hospitals with 301 beds total are Critical Access Hospitals (CAH), which serve rural populations and therefore are eligible to receive certain benefits from Medicare such as cost-based reimbursement for Medicare services (Rural Health Information Hub 2018); 5 hospitals with 498 beds total are Specialty Hospitals, which provide a specialized category of services (such as children’s hospitals, orthopedic hospitals, cancer hospitals, etc.). All of these hospitals can serve non-Medicare patients as well.
...
The 2018 Medicare Facility Pricing Amount is constructed for each CPT code (each procedure) at each hospital/facility from the 2018 Relative Value Files available through CMS. I then combine these Medicare prices with those available through NH Health Cost. For ten common procedures, I present the resulting boxplot distributions in Figure 2.1, which includes payments by the uninsured, private insurers, and Medicare, and in Figure 2.2, which only shows the payments made by private insurers. All price data presented reflect 2018 prices.
...
primary_data %>% 
  ...
  ggplot() +
  geom_boxplot(aes(group = `Hospital overall rating`, x = `Hospital overall rating`, y = percent_medicare)) +
...
stargazer(lm_hospital_compare, type = "html", omit = c(2:43), 
...
The primary source of data on hospital costs comes from [NH HealthCost](https://nhhealthcost.nh.gov/costs/select) which is a publicly available tool that provides insurer-hospital specific estimations of total costs (reimbursements) for a long list of outpatient services and procedures. All cost information is based on claims data collected in the New Hampshire Comprehensive Healthcare Information System which unfortunately I was unable to access directly. On the NH HealthCost website, the average amount each insurer pays for a given service at a given hospital is available, and it is reasonable to assume that this is roughly equivalent to the price each hospital and insurerance carrier have negotiated for each service. This allowed me to construct a data set containing the estimated price for each procedure and each provider-insurer combination for which data was available. 

### CMS Data
The Center for Medicaid and Medicare Services (CMS) provides several useful data sets that aid my analysis. The first is the Physician Fee Schedule (PFS) Relative Value files which allow me to calculate the Medicare reimbursement rates at each hospital for each service observed in the NH HealthCost data. This provides me with a baseline Medicare price for each service for each locality, and Medicare treats all of New Hampshire as one locality, meaning that all hospitals in New Hampshire are reimbursed the same amount for outpatient services. ***this may not be true for Critical Access Hospitals (CAHs) of which New Hampshire has 13 so I need to figure out at what rate Medicare reimburses these hospitals*** This allows me to standardize and compare how much above this baseline rate private insurers or the uninsured pay.The second dataset provided by CMS is the Hospital Compare data which provides measures of quality.
...
## Methodology
There are two stages to my analysis, and in both I choose to restrict my investigation to radiology services as these types of services (CT scans, MRIs, X-rays, etc.) can reasonably be assumed to have constant costs and quality within the same hospital. The first stage seeks to determine whether there is a state-wide statistical difference between the prices paid by the three major private insurers and the second stage evaluates whether hospitals with greater market power (defined as market share) extract higher prices.

### First Stage: Identifying Price Variation Patterns Across Major Private Insurers
