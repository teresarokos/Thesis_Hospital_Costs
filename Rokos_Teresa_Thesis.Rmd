...
However, this variation persists even when costs and quality are reasonably assumed to be fixed within the same hospital. Controlling for procedure, across hospital price variation accounts for 16.4% of the total variation but *within* hospital price variation accounts for a further 17.0% of the total variation in my data, similar to the findings Cooper et al. (2018).^[Cooper et al. (2018) find that about 20% of total price variation across the services they consider occurs within hospitals for the same procedure after controlling for hospital fixed effects, insurance plan characteristics, and patient characteristics.] For the same service at the same hospital, different payers are charged vastly different amounts. Figure 5 shows the different prices paid by the three major private payers in New Hampshire (Anthem, Harvard Pilgrim, and Cigna) at the same hospital for abdominal CT scans, on of the most expensive outpatient radiology procedures. For several of the hospitals in Figure 5, the difference between what the highest and lowest payers pay exceeds \$500.
...
\normalsize
Table 8 shows the unadjusted mean prices paid by each insurer for each procedure within the state of New Hampshire. Table 9 shows the percent of medicare these prices translate to when unadjusted. Because these values appear much higher than what is usually found in the relevate literature, I choose to deflate prices by a standardization factor such that the median price paid by the three major private insurers for each procedure is equivalent to 200% of the Medicare price for that procedure, such that:
$$
...
#   filter(insurer == "Medicare", `Hospital Type` != "Non-Hospital Providers", !cpt_code %in% c("29826", "42820", "43239", "45378", "99282", "99283"), 
#          locality == "NEW HAMPSHIRE") %>% 
#   count(procedure, medicare_price) %>% 
#   arrange(desc(medicare_price)) %>% 
#   rename("Procedure" = "procedure") %>% 
#   mutate(Procedure = str_replace_all(Procedure, "&", "/"),
#          Procedure = str_remove_all(Procedure, "\\s*\\([^\\)]+\\)"),
#          Procedure = str_replace_all(Procedure, "Emergency Department Visit - Low Complexity", "Emergency Room Visit - Low Level"),
...
kable(unadjusted_prices, format = "latex", caption = "Median Hospital Prices for Radiology Services by Payer (Unadjusted)", booktabs = T, digits = round(2),
      align = c("l", "r", "r", "r", "r", "r")) %>%
  kable_styling(latex_options = "scale_down") %>% 
  add_footnote("All prices presented reflect 2018 prices")
